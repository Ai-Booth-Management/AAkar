...
import './HealthDepartmentPanel.css';
...
export default function HealthDepartmentPanel() {
  ...
  const [projectForm, setProjectForm] = useState({
    id: '', name: '', type: 'Hospitals', contractor: '', executing_agency: '',
  ...
  const [detailsApprovalApprover, setDetailsApprovalApprover] = useState('Health Commissioner');
  ...
  const [modalApprovalApprover, setModalApprovalApprover] = useState('Health Commissioner');
  ...
  const [infraMetrics, setInfraMetrics] = useState({
    hospitals_completed: 0, hospitals_ongoing: 0,
    clinics_completed: 0, clinics_ongoing: 0,
    icu_beds_completed: 0, icu_beds_ongoing: 0,
    ventilators_completed: 0, ventilators_ongoing: 0,
    medicine_stock_completed: 0, medicine_stock_ongoing: 0,
    immunization_completed: 0, immunization_ongoing: 0
  });
  ...
  const fetchDistrictMetrics = async (silent = false) => {
    ...
    try {
      ...
      const json = await res.json();
      setInfraMetrics({
        hospitals_completed: json.hospitals_completed,
        hospitals_ongoing: json.hospitals_ongoing,
        clinics_completed: json.clinics_completed,
        clinics_ongoing: json.clinics_ongoing,
        icu_beds_completed: json.icu_beds_completed,
        icu_beds_ongoing: json.icu_beds_ongoing,
        ventilators_completed: json.ventilators_completed,
        ventilators_ongoing: json.ventilators_ongoing,
        medicine_stock_completed: json.medicine_stock_completed,
        medicine_stock_ongoing: json.medicine_stock_ongoing,
        immunization_completed: json.immunization_completed,
        immunization_ongoing: json.immunization_ongoing
      });
    ...
    try {
      const res = await authFetch(`${API_BASE}/district-metrics`, {
        ...
        body: JSON.stringify({
          ...
          hospitals_completed: parseFloat(infraMetrics.hospitals_completed) || 0,
          hospitals_ongoing: parseFloat(infraMetrics.hospitals_ongoing) || 0,
          clinics_completed: parseFloat(infraMetrics.clinics_completed) || 0,
          clinics_ongoing: parseFloat(infraMetrics.clinics_ongoing) || 0,
          icu_beds_completed: parseFloat(infraMetrics.icu_beds_completed) || 0,
          icu_beds_ongoing: parseFloat(infraMetrics.icu_beds_ongoing) || 0,
          ventilators_completed: parseFloat(infraMetrics.ventilators_completed) || 0,
          ventilators_ongoing: parseFloat(infraMetrics.ventilators_ongoing) || 0,
          medicine_stock_completed: parseFloat(infraMetrics.medicine_stock_completed) || 0,
          medicine_stock_ongoing: parseFloat(infraMetrics.medicine_stock_ongoing) || 0,
          immunization_completed: parseFloat(infraMetrics.immunization_completed) || 0,
          immunization_ongoing: parseFloat(infraMetrics.immunization_ongoing) || 0
        })
      });
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
    ...
    try {
      if (projectModal.type === 'add') {
        const payload = {
          ...
          type: projectForm.type || 'Hospitals',
  ...
  const handleModalSubmit = async () => {
    ...
    try {
      ...
      setModalApprovalApprover('Health Commissioner');
  ...
  const handleDetailsAction = async (actionType) => {
    ...
    try {
      ...
      setDetailsApprovalApprover('Health Commissioner');
        ...
        <div className="dept-form-header">
          <div>
            <h2 style={{ fontSize: 10, fontWeight: 900, color: 'var(--gray-400)', letterSpacing: '0.4em', textTransform: 'uppercase', marginBottom: 6 }}>
              Government of NCT of Delhi • Department of Health & Family Welfare
            </h2>
          ...
          <div className="card">
            {metricsLoading ? (
              ...
              <>
                <div className="dept-form-grid-3">
                  {[
                    { label: 'Hospitals Built (Units)', key: 'hospitals' },
                    { label: 'Primary Health Centers (Units)', key: 'clinics' },
                    { label: 'ICU Beds Upgraded (Units)', key: 'icu_beds' },
                  ].map(item => {
                    ...
                    return (
                      <div key={item.key} style={{ border: '1px solid var(--gray-200)', padding: 16 }}>
                        ...
                        <div className="dept-form-grid-2">
                          <div className="dept-form-group">
                            <label>{item.isPercent ? 'Level (%)' : 'Completed'}</label>
                            <input 
                              type="number" 
                ...
                <div className="dept-form-grid-3" style={{ marginTop: 16 }}>
                  {[
                    { label: 'Ventilators Installed (Units)', key: 'ventilators' },
                    { label: 'Medicine Stock Level (%)', key: 'medicine_stock', isPercent: true },
                    { label: 'Universal Immunization (%)', key: 'immunization', isPercent: true },
                  ].map(item => {
                    const completed = infraMetrics[`${item.key}_completed`];
                    const ongoing = infraMetrics[`${item.key}_ongoing`];
                    return (
              ...
              <div className="dept-modal-body">
                ...
                <div className="dept-form-grid-2">
                  <div className="dept-modal-field">
                    ...
                    <select className="dept-modal-input" value={projectForm.type} onChange={e => setProjectForm({ ...projectForm, type: e.target.value })}>
                      ...
                      <option value="Primary Health Centers">Primary Health Centers</option>
                      <option value="ICU Units">ICU Units</option>
                      <option value="Maternity Centers">Maternity Centers</option>
                      <option value="Diagnostics Centers">Diagnostics Centers</option>
                      <option value="Ambulance Procurement">Ambulance Procurement</option>
                      <option value="Specialty Clinics">Specialty Clinics</option>
                    </select>
                  </div>
                  <div className="dept-modal-field">
                    <label className="dept-modal-label">Officer In Charge</label>
                    <input className="dept-modal-input" value={projectForm.officer || ''} onChange={e => setProjectForm({ ...projectForm, officer: e.target.value })} placeholder="Dr. Rajesh" />
                  </div>
                </div>
                ...
                <div className="dept-form-grid-2">
                  <div className="dept-modal-field">
                    ...
                    <input className="dept-modal-input" value={projectForm.contractor} onChange={e => setProjectForm({ ...projectForm, contractor: e.target.value })} placeholder="e.g. HLL Lifecare" />
                  </div>
                  <div className="dept-modal-field">
                    <label className="dept-modal-label">Executing Agency</label>
                    <input className="dept-modal-input" value={projectForm.executing_agency} onChange={e => setProjectForm({ ...projectForm, executing_agency: e.target.value })} placeholder="e.g. Health Zone 1" />
                  </div>
                </div>
                  ...
                  {pendingApprovals.length === 0 ? (
                    ...
                    pendingApprovals.map(p => (
                      ...
                      >
                        ...
                        <td>{p.district}</td>
                        <td>{p.type}</td>
                        <td style={{ fontWeight: 600 }}>{p.officer || 'Health Officer'}</td>
      ...
      <div className="fade-in">
        ...
        {analyticsLoading || !analyticsData ? (
          ...
          <div className="dept-form-container" style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
            ...
            {/* Section 1: Health Facilities & Active Work Sites */}
            <div className="card">
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(350px, 1fr))', gap: 24 }}>
              ...
              <div className="card" style={{ display: 'flex', flexDirection: 'column', height: '100%' }}>
                <h3 style={{ fontSize: 13, fontWeight: 900, textTransform: 'uppercase', color: 'var(--gray-900)', letterSpacing: '0.06em', marginBottom: 16, borderBottom: '2px solid var(--gray-100)', paddingBottom: 10 }}>
                  Medical & Nursing Staff Availability
                </h3>
                <div style={{ display: 'flex', flexDirection: 'column', gap: 16, flex: 1, justifyContent: 'center' }}>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontWeight: 700, color: 'var(--gray-700)' }}>
                      ...
                      <span>{analyticsData.staff_availability?.doctors_on_duty} / {analyticsData.staff_availability?.doctors_total}</span>
                    </div>
                    <div className="progress-bar" style={{ height: 6, margin: 0 }}>
                      <div className="fill" style={{ 
                        width: `${(analyticsData.staff_availability?.doctors_on_duty / analyticsData.staff_availability?.doctors_total) * 100}%`,
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontWeight: 700, color: 'var(--gray-700)' }}>
                      ...
                      <span>{analyticsData.staff_availability?.nurses_on_duty} / {analyticsData.staff_availability?.nurses_total}</span>
                    </div>
                    <div className="progress-bar" style={{ height: 6, margin: 0 }}>
                      <div className="fill" style={{ 
                        width: `${(analyticsData.staff_availability?.nurses_on_duty / analyticsData.staff_availability?.nurses_total) * 100}%`,
                        backgroundColor: 'var(--blue-600)'
                      }} />
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, fontWeight: 700, color: 'var(--gray-700)' }}>
                      <span>Paramedical Staff Active</span>
                      <span>{analyticsData.staff_availability?.paramedical_active} / {analyticsData.staff_availability?.paramedical_total}</span>
                    </div>
                    <div className="progress-bar" style={{ height: 6, margin: 0 }}>
                      <div className="fill" style={{ 
                        width: `${(analyticsData.staff_availability?.paramedical_active / analyticsData.staff_availability?.paramedical_total) * 100}%`,
                        backgroundColor: 'var(--blue-600)'
                      }} />
                    </div>
                  </div>
