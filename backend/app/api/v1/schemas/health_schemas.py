"""Pydantic schemas for the Health Department Data Entry module.
...
class ProjectSchema(BaseModel):
    id: str
    name: str
    type: str  # "Hospitals" | "Primary Health Centers" | "ICU Units" | "Maternity Centers" | "Diagnostics Centers" | "Ambulance Procurement" | "Specialty Clinics"
    contractor: str = ""
...
class HealthMetricsSchema(BaseModel):
    hospitals: InfraMetricItemSchema = InfraMetricItemSchema()
    clinics: InfraMetricItemSchema = InfraMetricItemSchema()
    icu_beds: InfraMetricItemSchema = InfraMetricItemSchema()
    ventilators: InfraMetricItemSchema = InfraMetricItemSchema()
    medicine_stock: InfraMetricItemSchema = InfraMetricItemSchema()
    immunization: InfraMetricItemSchema = InfraMetricItemSchema()
...
class DistrictDataSchema(BaseModel):
    ...
    infrastructure: HealthMetricsSchema = HealthMetricsSchema()
...
class HealthDataFileSchema(BaseModel):
    """Full JSON file schema — used for reading/writing health.json & health_draft.json."""
    metadata: MetadataSchema = MetadataSchema()
    department: str = "Department of Health & Family Welfare"
    reporting_month: str = ""
...
class ProjectCreateSchema(BaseModel):
    name: str
    district: str
    type: str  # e.g., "Hospitals", "Primary Health Centers", etc.
    contractor: str = ""
    executing_agency: str = ""
